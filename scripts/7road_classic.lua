...
local server_id = userdata.server_id
...
local game_url = ""
for url, server_id_in_page in string.gmatch(index_page, [[<li.-class.-die10.->.-href="(.-)".-经典.-(%d).-<]]) do
    game_url = url
...
local assist_doc = agent:get(assist_url)
local result = string.match(assist_doc, [[<param.-name.-movie.-value=['"](.-)['"] />]])
